...
fecundity$plot <- as.factor(fecundity$plot)
...
for(cur_combo in sp_combos) {
  ...
  cur_fit <- brm(formula = Seeds ~ 1 + treatment + focal + sp1_count + sp2_count + (1|plot),
                 data = cur_fec, backend = "cmdstanr", family = lognormal, iter = 10000, adapt_delta = 0.9)
