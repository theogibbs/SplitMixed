...
# species combination as a fixed effect
...
melt_draws <- fec_draws %>%
  melt(id.vars = c(".chain", ".iteration", ".draw"))

ggplot(melt_draws, aes(x = value)) +
  geom_density() +
...
plPooledEffect <- ggplot(fec_draws, aes(x = exp(b_treatmentsplit),
                                          fill = after_stat(x < 0))) +
  ...
  theme(text = element_text(size=15),
        ...
        plot.title.position = "plot",
        plot.caption.position =  "plot") +
  labs(x = "Estimated Change in Number\nof Seeds from Clustered Competitors",
       y = "Density")
plPooledEffect

jpeg("./figs/SIFigPooledEffect.jpeg",
     width = 1250, height = 1000, res = 300)
plPooledEffect
dev.off()
