...
for(cur_combo in sp_combos) {
  ...
  cur_fit <- brm(formula = Seeds ~ 1 + treatment + (1|focal) + sp1_count + sp2_count,
                 data = cur_fec, backend = "cmdstanr",
...
fec_draws <- fec_draws %>% mutate(SpCombo = paste(sp1, sp2))
